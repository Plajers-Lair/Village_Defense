...
public class PluginEvents implements Listener {
  ...
  private void playKitSound(Kit kit, Player player) {
    ...
    } else if (kit instanceof PetsFriendKit) {
  ...
  public void onCreatureHurt(EntityDamageEvent event) {
    ...
    for (Arena arena : plugin.getArenaRegistry().getPluginArenas()) {
      if (!arena.getEnemies().contains(creature)
        && !arena.getWolves().contains(creature)) {
        continue;
      }
      event.setCancelled(false);
      creature.setCustomName(NewCreatureUtils.getHealthNameTagPreDamage(creature, event.getFinalDamage()));
    }
  }
